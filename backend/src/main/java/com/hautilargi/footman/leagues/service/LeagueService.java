package com.hautilargi.footman.leagues.service;
...
import com.hautilargi.footman.core.processing.MatchResults;
...
import com.hautilargi.footman.leagues.model.LeagueTable;
...
public class LeagueService {
    ...
    public MatchDayTableDto getTableForMatchDay(long season, int league, int matchday) {
        return new MatchDayTableDto(season,league,matchday,     
                generateTableForLeague(leagueRepository.findBySeasonNoAndIndex(season,league), season, matchday)
         );
    }
    ...
    private List<Match> generateLeagueSchedule(List<Team> teams, League league, long season) {
        if (teams.size() != 18) {
            throw new IllegalArgumentException(
                    "Exactly 18 Teams are needed. Something went wrong filling up with placeholders");
        }
    ...
    public void playMatchDay(int matchday) {
        // get current day
        System.out.println(
                "Playing Matchday " + matchday + " in season " + cs.getGlobalConfiguration().getCurrentSeason());
    ...
    public List<LeagueTableEntryDto> generateTableForLeague(League league, long season, int matchDay){
        ...
        List<LeagueTableEntryDto> tableAsDto = new ArrayList<>();
        int position=1;
        for(LeagueTableEntry entry :  table.getTableEntries()){
            tableAsDto.add(mapTableEntryToDto(entry, position++));
        }
        return tableAsDto;
    }

    private LeagueTableEntryDto mapTableEntryToDto(LeagueTableEntry entry, int position) {
        return new LeagueTableEntryDto(entry.getTeam().getId(), position, entry.getName(), entry.getGames(),
                entry.getWins(),entry.getDraws(),entry.getLosses(), entry.getGoals(), entry.getGoalsAgainst(), 
                entry.getTordifferenz(), entry.getPoints());
    }
